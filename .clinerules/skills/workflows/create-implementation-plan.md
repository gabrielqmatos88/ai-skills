...
description: 'Create a new implementation plan using directory structure for new features, refactoring existing code or upgrading packages, design, architecture or infrastructure.'
---
...
Your goal is to create a new implementation plan directory for `${input:PlanPurpose}`. Your output must be machine-readable, deterministic, and structured for autonomous execution by other AI systems or humans.
...
The plans must use a directory-based structure using markdown files. Each plan is stored in its own directory with separated concerns:
...
| `feedbacks.md` | Customer feedback collection - user-reported issues and requirements                         | ❌ NO - Must remain as-is (customer source of truth) |

### Findings.md Guidelines

This file stores relevant points to reduce data search during development execution:

- Development strategies and approaches
- File paths and their purposes
- What each file does and what it doesn't do
- Key discoveries during analysis
- Technical constraints and opportunities
- Code patterns and architecture decisions

### Feedbacks.md Guidelines

This file is **exclusively for human/customer input**:

- AI must NEVER modify this file
- Used to track customer-reported issues and requirements
- Serves as source of truth for customer expectations
- Should be reviewed before updating the plan
- Helps improve plan according to customer requirements
...
1. Create the plan directory with the appropriate naming convention
2. Create `plan.md` with the mandatory template structure (see below)
3. Create `findings.md` starting with a header and empty sections for findings
4. Create `feedbacks.md` as an empty markdown file (only header: `# Feedbacks`)

## Mandatory Template Structure (for plan.md)
...
[Approaches, patterns, and strategies to follow during implementation]
